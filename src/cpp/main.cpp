    ...
    int r, int g, int b, int a)
{
    ...
    *(img + coordinate + 1) = (unsigned char)(*(img + coordinate + 1) * inv_an + g * an);
    *(img + coordinate + 2) = (unsigned char)(*(img + coordinate + 2) * inv_an + b * an);
...
float computeRMSE(unsigned char* img_in, unsigned char* img_buff, int width, int height, int channels) {
    float sse = 0.0f;
...
void render(HCAParameters p, bool doPlot)
{
    ...
    //printf("width: %d height: %d channels: %d\n", template_width, template_height, png_channels);
    ...
    //stbi_uc* img_tmp = (stbi_uc*) calloc(template_width * template_height * png_channels * bpp, sizeof(unsigned char));
    //stbi_uc* img_out = (stbi_uc*) calloc(template_width * template_height * png_channels * bpp, sizeof(unsigned char));
...
#pragma omp parallel
                {
                    std::uniform_real_distribution<float> rdist(radiusMin, radiusMax); //recommended values: 0.5,20
...
#pragma omp critical
                ...
                if (doPlot)
                {
                    if (i % (ndiscs / 100) == 0)
                    {
                        float currentRmse = computeRMSE(img_tmp, img_template, template_width, template_height, template_channels);
    ...
    stbi_write_png(outputFilenameCStr, template_width, template_height, png_channels, img_tmp, template_width * png_channels);
    free(img_tmp);
    free(img_out);
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& name : names)
    {
        ...
        for (int i = 1; i < 5; i++)
        {
            ...
            {
                params.push_back({ .d = 20000 * i, .k = 100 * j,
                .alphaRange = alphaRange,.radiusRange = radiusRange,
                .colorRange = colorRange, .shape = Shape::Disc,
    ...
    printf("Generated %d parameter sets\n", params.size());
