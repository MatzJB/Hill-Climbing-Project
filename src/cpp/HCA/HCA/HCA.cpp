	...
	int height)
{
	...
	for (int j = min_y; j <= max_y; ++j)
	{
		...
		for (int i = min_x; i <= max_x; ++i)
		{
			...
			const float old_err = dr_old * dr_old + dg_old * dg_old + db_old * db_old;
...
int main()
{
	...
	for (int i = 0; i < 1000; i++) // add discs
	{
		...
		for (int j = 0; j < 100; j++) // test adding a disc
		{
			...
			//test_fun(pe_tmp);
			difference_metric = add_disc_diff_opt(img_template, img_tmp, template_channels, pe_tmp, template_width, template_height);

			if (difference_metric < pe_best.metric)
			{
				pe_best.x = x;
				pe_best.y = y;
				pe_best.radius = pe_tmp.radius;
				pe_best.col.r = pe_tmp.col.r;
				pe_best.col.g = pe_tmp.col.g;
				pe_best.col.b = pe_tmp.col.b;
				//pe_best.col.a = alpha;
				pe_best.col.a = pe_tmp.col.a;
